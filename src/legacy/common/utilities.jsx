...
export const GENERIC_SNOWFLAKE_RESOLVER_ABI = [
  {
    constant: false,
    inputs: [],
  ...
  {
    inputs: [
      { name: "_snowflakeName", type: "string" },
      { name: "_snowflakeDescription", type: "string" },
      { name: "_snowflakeAddress", type: "address" },
      { name: "_callOnSignUp", type: "bool" },
      { name: "_callOnRemoval", type: "bool" },
    ],
    payable: false,
    stateMutability: "nonpayable",
    type: "constructor",
  ...
  {
    constant: false,
    inputs: [{ name: "_snowflakeAddress", type: "address" }],
...
function getProvider(library) {
  if (isWeb3(library)) return library.currentProvider;
  else return library._web3Provider;
}

export function sendAsync(library, method, params, from) {
  return new Promise((resolve, reject) => {
    getProvider(library).sendAsync(
      { method, params, from },
      (error, result) => {
...
export async function signPersonal(library, address, message) {
  ...
  if (Buffer.isBuffer(message)) {
    ...
    encodedMessage = ensureHexPrefix(
...
async function sendTransaction(
  ...
  return method()
    .send({ from: address, value: transactionOptions.value })
